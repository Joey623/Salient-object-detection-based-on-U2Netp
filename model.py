...
class REBNCONV(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.conv_s1(hx)
        x2 = self.bn_s1(x1)
        xout = self.relu_s1(x2)


        return xout

# class REBNCONV5(nn.Module):
#     """1x1,5x5,1x1"""
#     def __init__(self, in_ch=64, out_ch=128):
#         super(REBNCONV5, self).__init__()
#
#         self.conv_s1 = nn.Conv2d(in_ch, out_ch, kernel_size=1, padding=2)
#         self.bn_s1 = nn.BatchNorm2d(out_ch)
#         self.relu_s1 = nn.ReLU(inplace=True)
#         self.conv_s2 = nn.Conv2d(out_ch, out_ch, kernel_size=5, padding=1)
...
#         x1 = self.conv_s1(hx)
#         x1_bn = self.bn_s1(x1)
#         x1_relu = self.relu_s1(x1_bn)
#         x2 = self.conv_s2(x1_relu)
#         x2_bn = self.bn_s2(x2)
#         xout = self.conv_s2(x2_bn)
#         return xout


class REBNCONV3(nn.Module):
    """1x1,3x3"""

    def __init__(self, in_ch=64, out_ch=64):
        super(REBNCONV3, self).__init__()
        self.conv_s1 = nn.Conv2d(in_ch, out_ch, kernel_size=1, padding=0)
        self.bn_s1 = nn.BatchNorm2d(out_ch)
        self.relu_s1 = nn.ReLU(inplace=True)
        self.conv_s2 = nn.Conv2d(out_ch, out_ch, kernel_size=3, padding=1)
        self.bn_s2 = nn.BatchNorm2d(out_ch)
        self.relu_s2 = nn.ReLU(inplace=True)
...
class UNet_Light(nn.Module):
    def __init__(self, in_ch=3, out_ch=1):
        ...
        self.inception1 = REBNCONV3(64, 64)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        # RSU6
        self.stage2 = RSU6(64, 16, 64)
        self.inception2 = REBNCONV3(64, 64)
        self.pool23 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        # RSU5
        self.stage3 = RSU5(64, 16, 64)
        self.inception3 = REBNCONV3(64, 64)
    ...
    def forward(self,x):
        ...
        # stage1
        hx1 = self.stage1(hx)
        hx11 = self.inception1(hx1)
        hx = self.pool12(hx1)


        # stage2
        hx2 = self.stage2(hx)
        hx22 = self.inception2(hx2)
        hx = self.pool23(hx2)

        # stage3
        hx3 = self.stage3(hx)
        hx33 = self.inception3(hx3)
        hx = self.pool34(hx3)
        ...
        hx3d = self.stage3d(torch.cat([hx4dup, hx3, hx33], 1))
        hx3dup = _upsample_like(hx3d, hx2)

        hx2d = self.stage2d(torch.cat([hx3dup, hx2, hx22], 1))
        hx2dup = _upsample_like(hx2d, hx1)
        hx1d = self.stage1d(torch.cat([hx2dup, hx1, hx11], 1))
